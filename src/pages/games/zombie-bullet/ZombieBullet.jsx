...
import { formatClock, moveToAdjacentLane, randomLaneDirection } from './gameUtils'
...
function resetVisualState(visual) {
  visual.fromLane = CENTER_LANE;
  visual.toLane = CENTER_LANE;
  visual.moveStartedAt = 0;
  visual.moveDuration = 210;
  visual.shotAt = Number.NEGATIVE_INFINITY;
  visual.shotLane = CENTER_LANE;
  visual.explosionAt = Number.POSITIVE_INFINITY;
  visual.explosionLane = CENTER_LANE;
...
function ZombieBullet() {
  ...
  const [game, setGame] = useState(() => createBulletGame(
      ...
      activeGame?.startedAt ?? 0
  ));
  const [remainingMs, setRemainingMs] = useState(initialTimeLimitMs);
  ...
  const currentLaneRef = useRef(CENTER_LANE);
  const laneDirectionRef = useRef(1);
  ...
  useEffect(() => subscribeGame((type, payload, metadata = {}) => {
    if (metadata.sequence && metadata.sequence <= processedSequenceRef.current) return;
    if (metadata.sequence) processedSequenceRef.current = metadata.sequence;
    ...
    const replayed = Boolean(metadata.replayed);
    ...
    if (type === "game:bullet_start") {
      ...
      currentLaneRef.current = CENTER_LANE;
      laneDirectionRef.current = randomLaneDirection();
      resetVisualState(visualRef.current);
      return;
    }
    ...
    if (type === "game:bullet_hit") {
      ...
      if (!replayed) {
        ...
        visual.shotAt = now;
        visual.shotLane = currentLaneRef.current;
        soundManager.shot();
        window.setTimeout(() => soundManager.hit(), 75);
        const movement = moveToAdjacentLane(currentLaneRef.current, laneDirectionRef.current);
        visual.fromLane = visualLane;
        visual.toLane = movement.lane;
        visual.moveStartedAt = now + 55;
        currentLaneRef.current = movement.lane;
        laneDirectionRef.current = movement.direction;
      }
      return;
    }
    ...
    if (type === "game:bullet_miss") {
      ...
      setVisibleFeedback(nextGame.feedback);
      if (!replayed) soundManager.error();
      return;
    }
    ...
    if (type === "game:clear") {
      ...
      const visualLane = visualLaneAt(visual, now);
      visual.fromLane = visualLane;
      visual.toLane = visualLane;
      visual.explosionLane = visualLane;
      visual.explosionProgress = clamp(1 - remainingMsRef.current / nextGame.timeLimitMs);
      visual.explosionAt = now + 115;
      visual.particlesBuilt = false;
      if (!replayed) window.setTimeout(() => soundManager.win(), 105);
      return;
    }
    ...
    if (type === "game:over") {
      ...
      if (!replayed) soundManager.lose();
    }
  }, { replay: true }), [room?.playerId, subscribeGame]);
  ...
  return <main className={`zombie-bullet-game game-shell phase-${phase}`} data-game-phase={phase}>
      ...
      <section className="game-stage" aria-label="巨大ゾンビ迎撃フィールド">
        ...
        {phase === "playing" && <div className="play-interface">
            <section className="question-card">
              ...
              <div className="player-turn-strip" aria-label="回答順">
                {game.players.map((player) => <span
                  className={`${player.player_id === game.currentPlayerId ? "is-current" : ""} ${player.player_id === room?.playerId ? "is-me" : ""}`}
                  key={player.player_id}
                >
                  {player.nickname}{player.player_id === room?.playerId ? "（あなた）" : ""}
                </span>)}
              </div>
            </section>
        ...
        {phase === "win" && <section className="stage-panel result-panel win-panel" aria-live="assertive">
            ...
            <div className="result-actions">
              <button className="primary-action" type="button" onClick={handleExit}>
                <span>タイトルへ戻る</span><small>LEAVE ROOM</small>
              </button>
            </div>
          </section>}
